...
- Titles in sentence case, no period. (Exception: a phrase the owner explicitly wants as a short declarative slogan can take periods - e.g. the `/our-innovations/` phase taglines "Better understanding. Better planning." - when they send it written that way.)
- Numbers, dates, and prices follow the conventions of the site language, consistently.
- **Reading the owner's copy submissions (recurring, 2026-09-10):** she sends project pitch rewrites as a markdown-ish block - `### ProjectName` as a label for each, and `**phrase**` around the words she wants emphasised. The `###` is just an identifier (match it to the project, don't turn it into a heading); the `**...**` means render that span as `<strong>` (the `/our-innovations/` project-card `<p>` is already `set:html` for exactly this - see `projects.js`). The project *name* she types in the `### heading` is often title-cased ("The Augmented Operative Report", "Expert Patients Masterclass") even when the live name is sentence case - that's heading-formatting habit, not a rename request; keep the existing name unless she says otherwise, and flag it in the reply so she can confirm. These pitches live per-phase in `site/src/data/projects.js` under each project's `en`/`fr`; a few (LiverSight, BOPCAM, Colette) also have an independent hardcoded copy in the homepage's featured rail (`HomeContent.astro`) that these edits do NOT reach.
