...
- The homepage's shared `main` gradient (see `site.css`) has its color stops set as percentages of the page's *total height*, not fixed pixel positions - so any edit that changes a section's height (trimmed copy, tightened gaps, moved or added content) also shifts which section lands on which color. Currently a single simple arc (white through the hero, easing to a cold blue by ~80%, where the solid-navy "Nous soutenir" section begins and takes over) - it replaced an earlier alternating multi-band version that read as too dark in places. After any homepage edit that changes content length, re-measure section boundaries as a % of `main`'s height and re-check where the gradient's stops now fall.
