...
- **Pages**: `index` (homepage: hero, context band, mission, projets phares, **qui sommes-nous** - a single rich chapter right after projets phares, covering la Chaire/recherche, l'environnement clinique du CHB, l'équipe (`#equipe` now lives inside it), et le mécénat -, **nous soutenir** (`#soutenir`, short/premium, real AP-HP donation link) + **nous contacter** (`#contact`, real email/address/LinkedIn de la Chaire, Web3Forms) - `#contact` is now the last editorial section before the footer), `innovations` (all 14 projects, tabbed by category: préopératoire/peropératoire/postopératoire/médico-économique - the definitive list lives in `site/src/data/projects.js`), `boptech` (the BOPtech platform), `publications`
...
- 2026-08-27 (seventh pack, same day) · The legacy "Notre approche" / "La vision : changer les pratiques du bloc opératoire" / "La méthode" (Prototyper/Codévelopper/Tester) block removed entirely - it had survived, unused-but-rendered, through several redesign passes at the end of the home. Nav's `/#approche` entry removed too (the "La Chaire" dropdown now has a single child, "Qui sommes-nous" - left as-is rather than flattened, since this pass's scope was explicitly deletion-only). `#contact` ("Nous contacter") is now the page's last editorial section before the footer. While cleaning nearby id-scoped CSS, also found and fixed a real, previously-undetected bug (see `source/brand/design.md`'s sixth "id beats class" variant): stray leftover rules were forcing "Qui sommes-nous" to a solid navy block and "Nous soutenir" to a light background since each section's own launch - both now render with their intended colors.
  **Open, awaiting the owner's clarification (flagged same day)**: the removal pack's instructions assumed a section called "Notre vision - De l'innovation au progrès" already existed between "Nos innovations" and "Qui sommes-nous" and explicitly warned not to touch it - no such section exists anywhere in this repo (confirmed via `git pull` + a global search). Either a companion pack that was meant to add it hasn't arrived yet, or there's a mix-up about what's already been integrated - don't assume it should be built from the removal pack's description alone; ask the owner first.
