...
- **Then publish without being asked — always, including visible redesigns.** Stage the files you touched explicitly by name (never a blind `git add -A`), commit with a clear message, push, and confirm the live site updated. Tell the owner in plain words: "saved and published". The owner confirmed explicitly (2026-08-28) that this is the permanent default, not a per-task grant — stop waiting for a "publie"/"go ahead" before pushing. Pause for approval only for the items in "Ask before" just below, or when you are genuinely unsure a change is what they meant.
...
- **Ask before**: deleting content, or anything touching money, accounts, or credentials. (Visible redesigns no longer need a pause — verify locally, then publish; see above.)
